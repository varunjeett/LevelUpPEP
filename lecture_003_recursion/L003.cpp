...
bool isValid(int r,int c, vector < vector <int> > &board){
    ...
    if ( r>=0 && c>=0 && r<board.size() && c<board[0].size() && board[r][c]==0 ) return true;
...
void rot_start(int r,int c, vector< vector<int> > &board){
    ...
    for(int i=0;i<4;i++){
        ...
        if(r_>=0 && c_>=0 && r_<board.size() && c_<board[0].size() && board[r_][c_]==1 ){ //picking only fresh from its neighbourhood
...
int rottenOranges_4DIR( vector < vector <int> > &board){
    ...
    for(i=0;i<board.size();i++){
        ...
        {
            if(board[i][j]==2){   //rotten and unvisted
              rot_start(i,j,board);  
            }
        }
    }
