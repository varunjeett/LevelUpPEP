...
public class L024 {
    ...
    public static Treenode LCA(Treenode root , int p , int q) {    
        ...
        if(p_ans.size()==0 || q_ans.size()==0) return null;      
    ...
    public static boolean LCA_byfind(Treenode root,int p , int q) {
        ...
        boolean right = LCA_byfind(root.left, p, q);
        if(ans!=null) {
            return true;
        }
    ...
    public static pair_ diameter02(Treenode root) {
        if(root=null) return new pair_(0,-1);  
    ...
    //just like the height of tree bas diameter kbhi kbhi update hora hai
    public static int dia_global=0;
    public static int diameter03(Treenode root) {
        ...
        dia_global=Math.max(dia_global,l_val+r_val+2);
